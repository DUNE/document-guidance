...
We have a standard table format for linking to interface documents in the DUNE docdb. In the first column, put the system to which you are interfacing. For the second column, we provide a macro to use to add linked references,  \verb|\intfcdoc{}{}|. The first parameter is the interface document's docdb number, and the second is its version number. Both parameters are required. 

The output of the macro is a hyperlink to the version of the document in the docdb and a citation.  The macro requires that the citation's label be of the form: \verb|bib:docdbNNNN|, where NNNN is the docdb number.
...
CISC & \intfcdoc{6787}{1}                \\ \colhline
... & \intfcdoc{nnnn}{n}                 \\ \colhline
(last item)& \intfcdoc{}{}               \\
\end{dunetable}
\end{verbatim}
